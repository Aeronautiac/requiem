<script lang="ts">
  // A modal, built on the native <dialog>. `showModal()` is what gives us the focus trap, Esc to
  // close, inertness of the page behind and ::backdrop — the four things a hand-rolled overlay gets
  // wrong, and the reason this needs no library.
  //
  // There is no trigger prop: a dialog is opened by setting `open`, and every call site already has
  // a Button to hand. One way to open it beats two.
