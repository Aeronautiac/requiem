<script lang="ts">
  ...
  // both. Usable as long as some outcome still has charges — or the ability has no pools at all,
  // which means no restriction.
  const usable = $derived(hasUi && (unlimited || successUsages > 0 || failureUsages > 0));
  ...
  <div class="flex items-center justify-between gap-3">
    <div class="flex flex-col gap-1">
      ...
      <span class="text-xs text-neutral-500">
        ...
        {/if}
        {#if !unlimited && baseReset > 0}· recharges every {baseReset}{/if}
        {#if !unlimited && resets > 0}· resets in {resets}{/if}
