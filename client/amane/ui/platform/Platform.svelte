<script lang="ts">
  // The platform screen: what a client shows when no game is joined. Two unrelated jobs,
  // deliberately kept apart because they use different credentials —
  //   - joining: a per-game key, which is what an ordinary player has.
  //   - administration: a platform key, which gates creating and destroying games and is
  //     nothing to do with any game's own keys. Only rendered when the host actually has a
  //     platform behind it (armonia does not), per context injection.
  import { getContext } from "svelte";
  import { PLATFORM_KEY, PlatformState } from "../../platform.svelte";

  const platform = getContext<PlatformState>(PLATFORM_KEY);
  ...
  // The credentials a freshly created game hands back. Shown once, here, because the server
  // never stores them in a retrievable form — losing this admin key means losing the game.
  let created = $state<{ game_id: number; admin_key: string } | null>(null);
  let endId = $state("");

  const joining = $derived(platform.phase.status === "joining");
  // A host with an implicit game has no credential to ask for.
  const implicit = platform.host.implicitGame !== undefined;

  function join(event: SubmitEvent) {
    event.preventDefault();
    const id = Number(gameId);
    if (!Number.isInteger(id) || id < 0 || key.trim() === "") return;
    void platform.join({ gameId: id, key: key.trim() });
  }

  async function createGame() {
    if (!platform.host.platform) return;
    adminBusy = true;
    adminError = "";
    created = null;
    try {
      created = await platform.host.platform.createGame(platformKey.trim());
  ...
  async function endGame() {
    ...
    adminError = "";
    try {
      await platform.host.platform.endGame(id, platformKey.trim());
      endId = "";
    } catch (e) {
      adminError = e instanceof Error ? e.message : String(e);
    } finally {
...
<div class="flex flex-1 items-center justify-center overflow-y-auto p-8">
  <div class="w-full max-w-md space-y-8">
    ...
    {#if platform.host.canQuit}
      <button
        class="text-xs text-neutral-500 underline"
        onclick={() => platform.host.quit()}
      >
        Quit
      </button>
    {/if}
