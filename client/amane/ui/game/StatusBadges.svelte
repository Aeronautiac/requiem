<script lang="ts">
  // The current viewer's own states, always visible rather than behind a button — being in custody
  // or under the radar changes what you should be doing right now, so it is status, not a list.
  //
  // Own states only, which is the whole shape of ActorState: what you know about anyone ELSE comes
  // from the event that announced it (a death, a kidnapping), never from here.
  ...
  const states = $derived(game.views.get(ui.viewer)?.states ?? 0);
