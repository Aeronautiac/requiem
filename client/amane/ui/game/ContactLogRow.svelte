<script lang="ts">
  // One entry of a contact log: a framed block holding a muted time column and a plain sentence.
  // Deliberately not an Announcement — a log is read by scanning dozens of these, so the row
  // carries no label of its own and leans on the accent colour instead.
  //
  // The parent resolves both ends to display strings (it owns the game context and the
  // ActorDisplay -> name logic); this is pure presentation, like Message and Announcement.
  ...
  // The verb wraps around the second name, so each row reads as a sentence rather than a record
  // to decode. Colour is scanning help only — the words carry it on their own.
...
<div class="px-4 py-1">
  <!-- The accent is the only thing distinguishing one kind of contact from another at a glance;
       the border and tint are what make a row a block rather than a line of output. -->
  <div
