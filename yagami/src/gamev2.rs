...
// no clean split between a shell and a harness
//
// here's why:
// if keys were shell level, then they wouldnt work across multiple timelines
// key creation is not deterministic, so we'd need to create them externally, and then inject them
// into a shell, but if shells dont do permissions enforcement, then there is no point in that.
//
// what problem(s) are we really trying to solve?
// time travel, time sandboxing, parallel timelines, server data extensions, consistency on reboot
//
// data extensions (keys, profiles) can hold a creation time, and can become invalid if you travel
// before their creation time, and perform some action.
// there aren't many data extensions on the server, so this isn't unmanageable.
//
// parallel timelines are just a timeline that only the admin can control. no keys here.
